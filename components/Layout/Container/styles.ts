...
export const StyledHTML = createGlobalStyle<StringProps>` 
  body {
    ...
    strong, .color-black {
      color: ${({ darkTheme }) => (darkTheme ? COLOR_WHITE : COLOR_DARK_PURPLE)};
      &:hover {
    ...
    pre {
      background-color: ${({ darkTheme }) =>
        darkTheme ? COLOR_BLACK : COLOR_LIGHT_CREAM};
      color: ${COLOR_BLACK};
    ...
    button {
        ...
        color: ${({ darkTheme }) => (darkTheme ? COLOR_DARK_PURPLE : COLOR_WHITE)};
    }

    h2, h5 {
      color: ${({ darkTheme }) => (darkTheme ? COLOR_LIGHT_GRAY : COLOR_DARK_PURPLE)};
    }
